...
- **Secrets**: only `ANTHROPIC_API_KEY`, read from `.env` via `python-dotenv`. It must be set before `uvicorn` starts; `--reload` does not reliably pick up `.env` changes without a full restart. `.env` is gitignored and was never committed, but on a shared/training server a real key still sits in plaintext in the working tree — restrict its file permissions (e.g. `chmod 600 .env`) and rotate the key after the workshop rather than leaving it live indefinitely.
